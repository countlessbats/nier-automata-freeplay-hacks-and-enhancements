...
// Tried and ruled out, recorded so none of it is attempted again:
//   the set-scanning query +0x7F4F80, answered no at its gameplay, label and
//     button-availability call sites (+0x42CD54, +0x7E0F7F, +0x89135F)
//   the leaf predicate isAutoChip(id) at +0x7E69D0, stubbed to return false
// Neither changed the corner readout or what L2 does, so neither is what the
// widget consults. The readout itself is drawn by +0x9870BA, which has no
// direct callers and no vtable slot, so the condition that shows it has to be
// found by watching the game rather than by reading it.
